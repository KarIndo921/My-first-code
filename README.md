...
It models a simplified Order-to-Cash flow: create a sales order, price the
lines with an optional discount, ship the order, run a credit check against
the customer's credit limit, and produce an invoice.
...
2. **Ask for a bug fix**: "What happens if `unit_price` is negative? Add
   validation and a test for it."
3. **Ask it to run tests**: "Run the test suite and fix anything that fails."
4. **Ask for a refactor**: "Split this into `models.py` and `order_to_cash.py`
   and update imports."
5. **Ask for a bigger feature**: "Add a `PurchaseOrder` and inventory check so
   an order can't ship more than what's in stock."
